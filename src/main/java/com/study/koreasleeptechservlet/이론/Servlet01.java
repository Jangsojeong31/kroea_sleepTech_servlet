...
public class Servlet01 {
    ...
    >> 웹 어플리케이션의 백엔드 역할을 하는 자바 클래스

    2. 서블릿 사용 목적
    : '요청을 처리하고 응답을 생성'하는 프로그램
    ...
    5. 서블릿과 스프링의 관계
    : Spring Framework는 내부적으로 서블릿 기반으로 만들어짐
    - 서블릿은 스프링의 기초가 되는 기술

    6. 용어 정리
    1) 서블릿: HTTP 요청 처리 자바 클래스
    2) 컨테이너: 서블릿을 실행시켜주는 환경 ex. 톰캣
    ...
    2) 스프링: DispatcherServlet(매니저)가 일처리를 지시
        >> 요청을 DispatcherServlet이 받아 컨트롤러, 서비스, 레포지토리 등의 역할을 분담하여 동작시킴
            + 결과를 모아 응답 전송
